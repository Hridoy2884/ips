<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    <section class="overflow-hidden bg-white py-11 font-poppins dark:bg-gray-800">
            ...
            <div class="flex flex-wrap -mx-4">
                ...
                <div class="w-full mb-8 md:w-1/2 md:mb-0" x-data="{ mainImage: '{{ url('storage', $product->images[0] ?? '') }}' }">
                    <div class="sticky top-0 z-50 overflow-hidden">
                        ...
                        <div class="flex flex-wrap hidden md:flex">
       ...
       <!-- Product Description -->
                        <div class="prose dark:prose-invert max-w-none mb-8 text-gray-700 dark:text-gray-400">
                            {!! $product->description !!}
                        </div>
    ...
    <button wire:click='buyNow({{ $product->id }})'
        class="w-full p-4 bg-green-500 rounded-md lg:w-2/5 dark:text-gray-200 text-gray-50 hover:bg-green-600 dark:bg-green-500 dark:hover:bg-green-700">
        <span wire:loading.remove wire:target='buyNow({{ $product->id }})'>Buy Now</span>
        <span wire:loading wire:target='buyNow({{ $product->id }})'>Processing...</span>
    </button>
                        </div>
