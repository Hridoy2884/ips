    ...
    <div class="px-4 py-4 mx-auto max-w-7xl lg:py-6 md:px-6">
      <div class="flex flex-wrap mb-24 -mx-3">
        <div class="w-full pr-2 lg:w-1/4 lg:block">
          ...
          <div class="p-5 mb-5 bg-white border border-green-200 rounded-xl shadow-sm">
            ...
            <ul class="space-y-3">
              ...
              <li class="flex items-center">
                ...
                <label for="{{ $category->slug }}" class="ml-3 text-green-800 text-base">{{ $category->name }}</label>
              </li>
              @endforeach
            </ul>
          </div>
            ...
            <ul class="space-y-3">
              ...
              <li class="flex items-center">
                <input type="checkbox" wire:model.live="selected_brands" id="{{ $brand->slug }}" value="{{ $brand->id }}"
                  class="w-4 h-4 text-green-600 focus:ring-green-500 border-green-300 rounded">
                <label for="{{ $brand->slug }}" class="ml-3 text-green-800 text-base">{{ $brand->name }}</label>
              </li>
              @endforeach
            </ul>
          </div>
          ...
          <div class="p-5 mb-5 bg-white border border-green-200 rounded-xl shadow-sm">
            ...
            <input type="range" wire:model.live="price_range"
              class="w-full h-1 mb-4 bg-green-100 rounded cursor-pointer" max="50000" value="30000" step="1000">
        ...
        <div class="w-full px-3 lg:w-3/4">
          <div class="px-3 mb-4">
            <div class="items-center justify-between hidden px-3 py-2 bg-green-100 md:flex rounded">
              <select wire:model.live="sort"
                class="block w-48 px-4 py-2 text-green-800 bg-green-50 border border-green-300 rounded-md focus:ring-green-500 focus:border-green-500">
          ...
          <div class="flex flex-wrap items-center">
            ...
            <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3">
              <div class="border border-green-200 rounded-xl shadow-md hover:shadow-lg transition duration-300 bg-white">
                <div class="relative bg-green-50 rounded-t-xl overflow-hidden">
                  <a href="/products/{{ $product->slug }}">
                    <img src="{{ url('storage',$product->images[0]) }}" alt="{{ $product->name }}" class="object-cover w-full h-56">
                  </a>
                </div>
                <div class="p-4">
                  <h3 class="text-lg font-semibold text-green-800">{{ $product->name }}</h3>
                  <p class="text-green-600 font-bold mt-2">{{ Number::currency($product->price,'BDT') }}</p>
                  <div class="mt-4 text-center">
                    <a wire:click.prevent='addToCart({{ $product->id }})'
                      ...
                      <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 16 16">
                        ...
                        <path d="M3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
                      </svg>
